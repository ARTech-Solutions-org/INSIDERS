        ...
        return (
          <button
            ...
            className={cn(
              ...
              isFilled ? "text-amber-500 drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]" : "text-white/20 hover:text-white/40"
            )}
          >
...
export default function PublicFeedback() {
  ...
  if (error || !event) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-6 font-sans">
        <div className="w-full max-w-md bg-white/5 backdrop-blur-xl p-10 rounded-3xl shadow-2xl border border-white/10 text-center space-y-5">
          ...
          <h2 className="text-2xl font-serif text-white tracking-wide">Link Unavailable</h2>
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-6 font-sans overflow-hidden relative">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-amber-900/20 via-[#050505] to-[#050505]" />
        
        <div className="w-full max-w-md bg-white/5 backdrop-blur-xl p-12 rounded-[2rem] shadow-2xl border border-white/10 text-center animate-in fade-in zoom-in duration-1000 relative z-10">
          <div className="w-24 h-24 bg-gradient-to-br from-amber-400 to-amber-600 text-white rounded-full flex items-center justify-center mx-auto mb-8 shadow-[0_0_30px_rgba(245,158,11,0.3)]">
            <Check className="w-10 h-10" strokeWidth={3} />
          </div>
          <h2 className="text-3xl font-serif text-white tracking-wide mb-4">Exceptional.</h2>
          <p className="text-zinc-400 leading-relaxed">
            Your feedback for <strong className="text-white font-medium">{event.title}</strong> has been secured. We appreciate your insights in elevating our standards.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-amber-500/30 relative pb-40">
      {/* Immersive Background */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-0 inset-x-0 h-[500px] bg-gradient-to-b from-amber-500/10 to-transparent" />
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-amber-600/20 blur-[120px] rounded-full mix-blend-screen" />
        <div className="absolute top-1/3 right-1/4 w-[30rem] h-[30rem] bg-blue-600/10 blur-[150px] rounded-full mix-blend-screen" />
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 pt-20">
        ...
        <header className="text-center mb-16">
          <div className={cn("transition-all duration-1000 ease-out transform", mounted ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0")}>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm mb-6">
              ...
              <span className="text-[10px] font-medium tracking-[0.2em] uppercase text-zinc-300">Event Debrief</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-serif text-transparent bg-clip-text bg-gradient-to-br from-white via-zinc-200 to-zinc-500 tracking-tight leading-tight mb-4">
              {event.title}
            </h1>
        ...
        <form onSubmit={(e) => e.preventDefault()} className="space-y-12">
          ...
          <div className={cn("transition-all duration-1000 ease-out transform delay-200", mounted ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0")}>
            <div className="bg-white/[0.02] backdrop-blur-2xl rounded-[2rem] p-8 sm:p-14 border border-white/[0.08] shadow-2xl relative overflow-hidden group">
              ...
              <h2 className="text-2xl md:text-3xl font-serif text-center text-white mb-10 tracking-wide">Overall Experience</h2>
              ...
              <div className="space-y-4 relative z-10">
                <label htmlFor="general-comments" className="text-xs font-medium tracking-widest uppercase text-zinc-500 pl-4">Executive Summary</label>
                <Textarea
                  id="general-comments"
                  placeholder="What details defined your experience?"
                  className="bg-black/40 border-white/10 resize-none min-h-[140px] rounded-3xl focus-visible:ring-1 focus-visible:ring-amber-500/50 p-6 text-base placeholder:text-zinc-600 text-white transition-all duration-300"
          ...
          {hasTeams && generalRating > 0 && (
            <div className="animate-in fade-in slide-in-from-bottom-12 duration-1000 space-y-12 pt-8">
              <div className="text-center px-4 flex flex-col items-center">
                ...
                <h2 className="text-2xl font-serif text-white tracking-wide">Team Performance</h2>
                <p className="text-zinc-400 text-sm mt-3 max-w-md mx-auto">Evaluate specific units or individuals who made an impact.</p>
              </div>

              {event.teams.map((team, index) => (
                <div key={team.id} className="bg-white/[0.02] backdrop-blur-xl rounded-[2rem] p-6 sm:p-10 border border-white/[0.05] shadow-xl space-y-8 animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${index * 150}ms`, animationFillMode: 'both' }}>
                  {/* Team Header */}
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pb-8 border-b border-white/5">
                    <div className="text-center sm:text-left">
                      <h3 className="text-xl font-medium text-white tracking-wide">{team.name}</h3>
                      <p className="text-zinc-500 text-xs tracking-widest uppercase mt-2">{team.ushers.length} {team.ushers.length === 1 ? 'MEMBER' : 'MEMBERS'}</p>
                    </div>
                    <div className="bg-black/50 border border-white/5 rounded-full px-6 py-4 shadow-inner">
                      <StarRating 
                  ...
                  {teamRatings[team.id]?.rating > 0 && (
                    <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-2">
                      <Textarea
                        ...
                        className="bg-black/40 border-white/10 resize-none rounded-2xl focus-visible:ring-1 focus-visible:ring-amber-500/50 text-sm text-white placeholder:text-zinc-600 p-5"
                  ...
                  {team.ushers.length > 0 && (
                    <div className="pt-4">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500 mb-6 px-2 flex items-center gap-4">
                        <span>Individuals</span>
                        <span className="h-px flex-1 bg-white/5" />
                      </p>
                      <div className="grid gap-4 sm:grid-cols-2">
                        {team.ushers.map((usher) => (
                          <div key={usher.id} className="group bg-black/20 hover:bg-white/[0.03] rounded-2xl p-5 transition-all duration-300 border border-transparent hover:border-white/10">
                            <div className="flex justify-between items-center gap-3">
                              <div className="flex items-center gap-3 overflow-hidden">
                                <div className="w-10 h-10 rounded-full bg-zinc-800 border border-white/10 flex-shrink-0 flex items-center justify-center text-zinc-400 group-hover:text-amber-500 group-hover:border-amber-500/30 transition-colors">
                                  <User className="w-4 h-4" />
                                </div>
                                <span className="font-medium text-zinc-300 text-sm truncate group-hover:text-white transition-colors" title={usher.name}>{usher.name}</span>
                              </div>
                              <StarRating 
                            ...
                            {usherOverrides[usher.id]?.rating > 0 && (
                              <div className="mt-4 animate-in fade-in duration-500">
                                <Textarea
                                  ...
                                  className="bg-black/60 border-white/5 resize-none min-h-[80px] rounded-xl text-sm text-white placeholder:text-zinc-600 focus-visible:ring-1 focus-visible:ring-amber-500/50 p-4"
      ...
      {/* Floating Action Bar */}
      <div className={cn(
        "fixed bottom-0 left-0 right-0 p-6 sm:p-8 bg-gradient-to-t from-[#050505] via-[#050505]/95 to-transparent flex justify-center pointer-events-none transition-all duration-700 z-50",
        generalRating > 0 ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"
      )}>
        <Button 
          onClick={() => handleSubmit()}
          size="lg" 
          className="w-full max-w-md rounded-full h-16 text-base font-medium shadow-[0_10px_40px_rgba(245,158,11,0.15)] pointer-events-auto bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white transition-all duration-300 active:scale-[0.98] border border-amber-400/20"
